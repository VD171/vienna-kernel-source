	...
	int force_lcm_update)
{
	...
	if (mtk_drm_helper_get_opt(priv->helper_opt, MTK_DRM_OPT_SPHRT)) {
		...
		if (mtk_crtc->cur_usage == DISP_OPENING) {
			if (mtk_state->prop_val[CRTC_PROP_USER_SCEN] & USER_SCEN_ASYNC_EARLY_POWER_ON) {
			    DDPINFO("%s enter async power on\n", __func__);
		    } else {
...
static void mtk_dsi_encoder_enable(struct drm_encoder *encoder)
{
	...
	if (mtk_state->prop_val[CRTC_PROP_USER_SCEN] & USER_SCEN_ASYNC_EARLY_POWER_ON) {
	    ...
	    mtk_crtc->hold_top_clk_by_early_power_on = true;
		DDPINFO("%s enter early power on top clk enable", __func__);
	}
