...
static int do_algorithm(struct mtk_charger *info)
{
	...
	if (is_basic == true) {
		...
		if (info->en_cts_mode) {
			// close power path
			if (!info->en_power_path) {
				mtk_charger_force_disable_power_path(info, true);
				chr_err("%s: close power path\n", __func__);
			// open power path
			} else if (info->en_power_path) {
				mtk_charger_force_disable_power_path(info, false);
				chr_err("%s: open power path\n", __func__);
			}
		}
...
static const struct mmi_mux_configure config_mmi_mux[MMI_MUX_CHANNEL_MAX] = {
	...
	[MMI_MUX_CHANNEL_WLC_OTG] = MMI_MUX(MMI_DVCHG_MUX_DISABLE, MMI_DVCHG_MUX_OTG_WLC_OPEN, true, false, true, false),
#else
...
#ifdef CONFIG_MOTO_WLS_CP_OTG_REVERSE_SWITCH
	[MMI_MUX_CHANNEL_WLC_FW_UPDATE] = MMI_MUX(MMI_DVCHG_MUX_DISABLE, MMI_DVCHG_MUX_OTG_WLC_OPEN, false, false, false, false),
#else
