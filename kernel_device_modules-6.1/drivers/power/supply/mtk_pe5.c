...
#define PE50_IBAT_FCCTAGT_GAP_STEP3    6
#define PE50_IBAT_FCCTAGT_GAP_LEVEL2  4000
#define PE50_IBAT_FCCTAGT_GAP_STEP2    4
#define PE50_IBAT_FCCTAGT_GAP_LEVEL1  3000
#define PE50_IBAT_FCCTAGT_GAP_STEP1    3
#define PE50_IBAT_FCCTAGT_GAP_STEP_DEFT  1
...
int pe5_set_operating_mode(struct pe50_algo_info *info, enum pe50_cp_op_mode cp_op_mode) {
	struct pe50_algo_data *data = info->data;
	int ret = 0;

	if(data->is_dvchg_exist[PE50_DVCHG_MASTER]) {
		ret = pe5_hal_set_operating_mode(info->alg, to_chgidx(PE50_DVCHG_MASTER), cp_op_mode);
		if(ret <0) {
	...
	if(data->is_dvchg_exist[PE50_DVCHG_SLAVE]) {
		ret = pe5_hal_set_operating_mode(info->alg, to_chgidx(PE50_DVCHG_SLAVE), cp_op_mode);
		if(ret <0) {
	...
	if (data->ita_measure + ita_gap_per_vstep > idvchg_lmt ||
		...
		if(data->cp_op_mode_curr == CP_4_1_MODE ) {
			if(data->mmi_max_ibat - ibat > PE50_IBAT_FCCTAGT_GAP_LEVEL3)
				...
				vstep_cnt = min(vstep_cnt, PE50_IBAT_FCCTAGT_GAP_STEP_DEFT);
		}
		vta += auth_data->vta_step * (vstep_cnt + 1);
		vta = min(vta, (u32)auth_data->vcap_max);
...
static bool pe50_is_ta_rdy(struct pe50_algo_info *info)
{
	...
	if (!data->ta_ready) {
		...
		if (ret < 0 || (desc->charge_pump_op_mode_max_support == CP_4_1_MODE && auth_data->ita_max < VADPT_PPS_D4CP_THRE_CURRENT)) {
